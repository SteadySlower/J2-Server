...
@Injectable()
export class SyncPushHandlersService {
  constructor(private syncDeletion: SyncDeletionService) {}
  ...
  async processSchedules(
    ...
    for (const s of payload.schedules?.updated ?? []) {
      ...
      if (!existing) throw new NotFoundException(`Schedule ${s.id} not found`);
      if (existing.userId !== userId)
        throw new ForbiddenException('Access denied');
      if (existing.updatedAt > new Date(s.updated_at))
        throw new ConflictException(CONFLICT_MESSAGE);
  ...
  async processWordBooks(
    ...
    for (const wb of payload.word_books?.updated ?? []) {
      ...
      if (!existing) throw new NotFoundException(`WordBook ${wb.id} not found`);
      if (existing.userId !== userId)
        throw new ForbiddenException('Access denied');
      if (existing.updatedAt > new Date(wb.updated_at))
        throw new ConflictException(CONFLICT_MESSAGE);
  ...
  async processKanjis(
    ...
    userId: string,
    payload: PushPayload,
    kanjiIdMap: Record<string, string>,
  ) {
  ...
  async processWords(
    ...
    for (const w of payload.words?.created ?? []) {
      ...
      if (!book) throw new NotFoundException(`WordBook ${w.book_id} not found`);
      if (book.userId !== userId) throw new ForbiddenException('Access denied');
  ...
  async processWordKanji(
    ...
    for (const wk of payload.word_kanji?.created ?? []) {
      ...
      if (!word) throw new BadRequestException(`Word ${wk.word_id} not found`);
  ...
  async processKanjiKanjiBook(
    ...
    for (const kkb of payload.kanji_kanji_book?.deleted ?? []) {
      ...
      if (kanji && kanji.userId === userId && book && book.userId === userId) {
